...
if ($_POST) {
    ...
    $query = "SELECT overallIncorrectSpanish.wordID, testSpanishWord.word, testSpanishWord.translation FROM "
            . "overallIncorrectSpanish JOIN testSpanishWord ON overallIncorrectSpanish.wordID=testSpanishWord.wordID ";
    $results = $mysqli->query($query);

    $rows = $results->fetch_assoc();

    $correct_word = $rows['word'];
...
//   $_SESSION['userword']=$userword;
    ...
    if (trim(mb_strtolower($correct_choice)) == trim(mb_strtolower($userinput))) {
        ...
        $query = "INSERT INTO testSpanishWord(word, translation, pronunciation)"
                . "VALUES('$userinput', '$userinput', '$userinput')";
        $result = $mysqli->query($query);
        
        //getting id
        $query = "SELECT * FROM testSpanishWord WHERE word = '$userinput'";
        $output = $mysqli->query($query);
        $id_result = $output->fetch_assoc();
        $correct_id = $id_result['wordID'];
        
        //inserting id into choice
        $query = "INSERT INTO testSpanishChoice(wordID, questionID, is_correct)"
                . "VALUES ($correct_id, $number, '0')";
        $results = $mysqli->query($query);
